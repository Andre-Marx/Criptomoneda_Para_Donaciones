...
def test_transaction_exceeds_balance():
    with pytest.raises(Exception, match = 'El monto es mayor al balance'):
...
def test_invalid_transaction_with_invalid_outputs():
    ...
    with pytest.raises(Exception, match = 'Valores de salida de transacción inválidos'):
...
def test_valid_transaction_with_invalid_signature():
    ...
    with pytest.raises(Exception, match = 'Firma Inválida'):
        Transaction.is_valid_transaction(transaction)
